...
const TRANSLATIONS = {
  ...
  as: {
    skipContent: "মুখ্য বিষয়বস্তুলৈ যাওক",
    screenReader: "স্ক্ৰীণ ৰীডাৰ প্ৰৱেশাধিকাৰ",
    sitemap: "ছাইটমেপ",
    navDashboard: "ঘৰ / ডেচবৰ্ড",
    navAnalyzer: "প্ৰতিবেদন বিশ্লেষক",
    navHeatmap: "বিপদ হিটমেপ",
    navLSR: "জীৱন ৰক্ষাকাৰী নিয়ম",
    navClusters: "পেটাৰ্ণ ক্লাষ্টাৰ",
    navReports: "প্ৰতিবেদন ভঁৰাল",
    navGovernance: "বিধিগত নিৰীক্ষণ আৰু নীতি",
    dashTitle: "এইচএছএছই কমাণ্ড ডেচবৰ্ড",
    dashSub: "এছআইএফ পূৰ্বসূৰী নিৰীক্ষণ · অসম আৰু ৰাজস্থান ক্ষেত্ৰ সঞ্চালন · প্ৰকৃত সময়ৰ এআই বিশ্লেষণ"
  }
};
...
function setLanguage(lang) {
  currentLang = lang;
  ['En', 'Hi', 'As'].forEach(k => {
    const btn = document.getElementById(`langBtn${k}`);
    if (btn) btn.classList.toggle('active', lang === k.toLowerCase());
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Initialize Government Server Sentinel & Telemetry Stream
  initServerSentinel();
  animateKPICounters();
...
// ============================================================
//  AUTHENTIC GOVERNMENT OF INDIA & DYNAMIC SERVER LOGIC
// ============================================================

// 1. Official Government Verification Drawer
function toggleGovVerifyDrawer() {
  const drawer = document.getElementById('govVerifyDrawer');
  const btn = document.getElementById('govVerifyToggleBtn');
  if (!drawer) return;
  const isOpen = drawer.classList.toggle('open');
  if (btn) {
    btn.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
    btn.innerHTML = isOpen ? `<span>Close verification details</span> ▴` : `<span>Here's how you know</span> ▾`;
  }
}

// 2. GIGW Theme Switcher (Default, Dark, High-Contrast)
...
function setTheme(theme) {
  ...
  ['Default', 'Dark', 'HC'].forEach(k => {
    const btn = document.getElementById(`themeBtn${k}`);
    if (btn) {
      btn.classList.toggle('active',
        (k === 'Default' && theme === 'default') ||
        (k === 'Dark' && theme === 'dark') ||
        (k === 'HC' && theme === 'high-contrast')
      );
    }
  });
  showToast(`Active Theme: ${theme === 'high-contrast' ? 'GIGW High Contrast' : theme.toUpperCase()}`);
}

// 3. Low-Bandwidth / Rig-Link Mode for Remote Drilling Operations
let rigModeActive = false;
function toggleLowBandwidthMode() {
  rigModeActive = !rigModeActive;
  document.body.classList.toggle('low-bandwidth-mode', rigModeActive);
  const btn = document.getElementById('rigModeBtn');
  const txt = document.getElementById('rigModeText');
  if (btn) btn.classList.toggle('active', rigModeActive);
  if (txt) txt.textContent = rigModeActive ? 'Rig Mode: ACTIVE (Low-BW)' : 'Rig Mode: Normal';
  showToast(rigModeActive
    ? '⚡ Rig-Link Active: Payloads & animations optimized for remote drilling rigs'
    : 'Standard Bandwidth Mode Restored'
  );
}

// 4. Server Sentinel: Live Telemetry & Server Load Monitor
const RIG_TELEMETRY_EVENTS = [
  "[11:47:04] Ingested Rig-14 Duliajan · Gas sensors verified · SIF Risk: Low (14/100)",
  "[11:47:20] Moran GGS-2 · Hot Work PTW active · LOTO verified on line · Safe",
  "[11:47:38] Jaisalmer Wellpad-07 · Ambient Heat Advisory 41°C · Shift rotated",
  "[11:47:55] Digboi Derrick-4 · Height fall arrest inspection completed · Compliant",
  "[11:48:12] Naharkatia Compressor Station · Vibration analysis nominal · No SIF flags",
  "[11:48:30] Sibsagar Wellsite #5 · Electrical isolation tag audit passed · OISD compliant",
  "[11:48:46] Baghjan EPS · Gas containment sensor network active · Zero deviation"
];

let streamEventIdx = 0;
let streamTimer = null;
let streamActive = true;

function initServerSentinel() {
  updateStreamTicker();
  if (streamTimer) clearInterval(streamTimer);
  streamTimer = setInterval(updateStreamTicker, 4000);
}

function updateStreamTicker() {
  if (!streamActive) return;
  const ticker = document.getElementById('streamTickerText');
  if (ticker) {
    ticker.textContent = RIG_TELEMETRY_EVENTS[streamEventIdx % RIG_TELEMETRY_EVENTS.length];
    streamEventIdx++;
  }
  // Subtle realistic modulation of server load
  const loadBadge = document.getElementById('sentinelLoadBadge');
  if (loadBadge && !isBurstTesting) {
    const cpu = (2.1 + (Math.random() * 0.9)).toFixed(1);
    loadBadge.textContent = `${cpu}% CPU · Optimal`;
  }
}

function toggleLiveTelemetryStream() {
  streamActive = !streamActive;
  const dot = document.getElementById('streamPulseDot');
  const icon = document.getElementById('btnStreamPauseIcon');
  const txt = document.getElementById('btnStreamPauseText');
  if (dot) dot.style.animation = streamActive ? 'pulseGreen 2s infinite' : 'none';
  if (icon) icon.textContent = streamActive ? '⏸' : '▶';
  if (txt) txt.textContent = streamActive ? 'Pause Feed' : 'Resume Feed';
  showToast(streamActive ? 'Live Telemetry Feed Resumed' : 'Live Telemetry Feed Paused');
}

let isBurstTesting = false;
function simulateHighLoadBurst() {
  if (isBurstTesting) return;
  isBurstTesting = true;
  const loadBadge = document.getElementById('sentinelLoadBadge');
  const throughput = document.getElementById('sentinelThroughputVal');
  const ticker = document.getElementById('streamTickerText');

  if (loadBadge) {
    loadBadge.style.background = '#FEF3C7';
    loadBadge.style.color = '#92400E';
    loadBadge.style.borderColor = '#F59E0B';
    loadBadge.textContent = '8.7% CPU · Load Surge Absorbed';
  }
  if (throughput) throughput.textContent = '14,200 req/s · 18ms';
  if (ticker) ticker.textContent = '⚡ Ingested 500-report incident telemetry burst in 42ms · 0 dropped packets';

  showToast('⚡ 500-Report Telemetry Burst Processed in 42ms without server throttling!');

  setTimeout(() => {
    if (loadBadge) {
      loadBadge.style.background = '';
      loadBadge.style.color = '';
      loadBadge.style.borderColor = '';
      loadBadge.textContent = '2.4% CPU · Optimal';
    }
    if (throughput) throughput.textContent = '5,240 req/s · 14ms';
    isBurstTesting = false;
  }, 4500);
}

// 5. Instant Dashboard Search & Filtering
let activeDashboardTag = 'All';
function handleDashboardSearch(query) {
  ...
  items.forEach(card => {
    ...
    const matchesTag = activeDashboardTag === 'All' || text.includes(activeDashboardTag.toLowerCase());
  ...
  const allCountBadge = document.getElementById('countAlertsAll');
  if (allCountBadge && q) allCountBadge.textContent = matchCount;
}
...
function setDashboardFilterTag(tag) {
  activeDashboardTag = tag;
  document.querySelectorAll('.filter-chip-btn').forEach(btn => {
    btn.classList.toggle('active', btn.textContent.trim().toLowerCase() === tag.toLowerCase());
  });
  const input = document.getElementById('dashboardSearchInput');
  handleDashboardSearch(input ? input.value : '');
  showToast(`Filter applied: ${tag}`);
}

// 6. Ticker Marquee Accessibility Controls
let marqueePaused = false;
function toggleNoticePause() {
  ...
  if (btn) btn.textContent = marqueePaused ? '▶' : '⏸';
  showToast(marqueePaused ? 'Safety Notice Ticker Paused' : 'Safety Notice Ticker Resumed');
}

function prevNotice() {
  const marquee = document.getElementById('noticeMarquee');
  if (marquee) marquee.scrollLeft -= 200;
}
...
// 7. Animated Counter Roll-up for KPIs
function animateKPICounters() {
  const targets = [
    { id: 'kpiTotal', target: 3000 },
    { id: 'kpiSIF', target: 900 },
    { id: 'kpiCritical', target: 270 }
  ];
