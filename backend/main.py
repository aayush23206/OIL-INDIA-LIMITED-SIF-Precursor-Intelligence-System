...
def _find_dataset_path() -> Path:
    candidates = [
        BASE_DIR / "data" / "oil_sif_full_3000_mapped.json",
        BASE_DIR / "backend" / "data" / "oil_sif_full_3000_mapped.json",
        BASE_DIR.parent / "backend" / "data" / "oil_sif_full_3000_mapped.json",
        BASE_DIR / "oil_sif_full_3000_mapped.json",
        BASE_DIR.parent / "oil_sif_full_3000_mapped.json",
    ]
    for c in candidates:
        if c.exists():
            return c
    return BASE_DIR / "data" / "oil_sif_full_3000_mapped.json"
...
def get_all_reports(origin: str = Query(default="all", pattern="^(all|new|historical)$"), limit: int = 50, skip: int = 0):
